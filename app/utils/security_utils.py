...
class SecurityUtils:
    ...
    SENSITIVE_PATTERNS = [
        ...
        (r'[A-Fa-f0-9]{8}-[A-Fa-f0-9]{4}-[A-Fa-f0-9]{4}-[A-Fa-f0-9]{4}-[A-Fa-f0-9]{12}', '***ENTSOE-KEY-HIDDEN***'),
        
        # Shelly auth keys (typically base64-like strings)
        (r'[A-Za-z0-9+/]{32,}={0,2}', lambda m: f"***SHELLY-KEY-{m.group()[:4]}...{m.group()[-4:]}***" if len(m.group()) > 20 else '***HIDDEN***'),
